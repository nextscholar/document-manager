...
def _vector_param(embedding: List[float]) -> str:
    """
    Serialize a Python list[float] into a pgvector literal accepted by CAST(... AS vector).
...
def search_docs_stage1(
    ...
    filter_sql = ""
    params = {
        # IMPORTANT: use CAST(:embedding AS vector) in SQL; this value must be a pgvector literal
        "embedding": _vector_param(query_embedding),
        "query": query,
    ...
    sql = text(
        ...
        WITH doc_vector AS (
            SELECT rf.id,
                   1.0 - (rf.doc_embedding <=> CAST(:embedding AS vector)) as vector_score,
                   ROW_NUMBER() OVER (ORDER BY rf.doc_embedding <=> CAST(:embedding AS vector)) as vector_rank
            FROM raw_files rf
            WHERE rf.doc_embedding IS NOT NULL
            {filter_sql}
            ORDER BY rf.doc_embedding <=> CAST(:embedding AS vector)
            LIMIT 100
        ),
        doc_keyword AS (
            SELECT rf.id,
                   ...
                   ROW_NUMBER() OVER (
                        ORDER BY ts_rank_cd(rf.doc_search_vector, plainto_tsquery('english', :query))
                        DESC NULLS LAST
                   ) as keyword_rank
            FROM raw_files rf
...
def search_chunks_stage2(
    ...
    Uses RRF for combining vector and keyword scores.
    ...
    sql = text(
        ...
        WITH vector_ranked AS (
            SELECT e.id,
                   ...
                   ROW_NUMBER() OVER (ORDER BY e.embedding <=> CAST(:embedding AS vector)) as vector_rank
            FROM entries e
        ...
        keyword_ranked AS (
            SELECT e.id,
                   ...
                   ROW_NUMBER() OVER (
                        ORDER BY ts_rank_cd(e.search_vector, plainto_tsquery('english', :query))
                        DESC NULLS LAST
                   ) as keyword_rank
            FROM entries e
            WHERE e.file_id = ANY(:doc_ids)
    ...
    score_map = {
        r[0]: {
            "vector_score": r[1],
            "keyword_score": r[2],
            "combined_score": r[3] if len(r) > 3 else r[2],
        }
        for r in results
    }
...
def search_two_stage(
    ...
    if not query_embedding:
        ...
        return {"entries": [], "docs": [], "stats": {"error": "embedding_failed"}}
    ...
    if mode == SEARCH_MODE_VECTOR:
        ...
        if filters:
            ...
            if filters.get("uploaded_by"):
                ...
                stmt = stmt.filter(
                    or_(
                        RawFile.uploaded_by == filters["uploaded_by"],
